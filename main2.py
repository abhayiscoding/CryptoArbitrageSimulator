...
class CryptoArbitrageSimulator:
    ...
    def setup_plots(self):
        ...
        self.bid_bars = self.ax1.bar(range(len(self.exchanges_list)), [0]*len(self.exchanges_list), 
                                    color='red', alpha=0.6, label='Bid')
        self.ask_bars = self.ax1.bar(range(len(self.exchanges_list)), [0]*len(self.exchanges_list), 
                                    color='green', alpha=0.6, label='Ask', bottom=[0]*len(self.exchanges_list))
        
        self.ax1.set_title('Current Bid/Ask Prices by Exchange')
        self.ax1.set_xticks(range(len(self.exchanges_list)))
        self.ax1.set_xticklabels([ex.capitalize() for ex in self.exchanges_list])
        self.ax1.set_ylim(110000 , 120000)  # Adjust based on current Bitcoin price range
    ...
    def update_plots(self, frame=None): #######################Difference####
        """Update the matplotlib plots"""
        # Update bid/ask bars
        for i, exchange in enumerate(self.exchanges_list):
            if self.prices[exchange]['bid'] > 100 and self.prices[exchange]['ask'] > 100:
                # Display actual prices (not normalized)
                self.bid_bars[i].set_height(self.prices[exchange]['bid'])
                self.bid_bars[i].set_y(0) 
                # self.ask_bars[i].set_height(self.prices[exchange]['ask'] - self.prices[exchange]['bid'])
                spread_height = max(0, self.prices[exchange]['ask'] - self.prices[exchange]['bid'])
                self.ask_bars[i].set_height(spread_height)
                self.ask_bars[i].set_y(self.prices[exchange]['bid'])

        # Update balance text
        avg_price = sum(self.prices[ex]['last'] for ex in self.exchanges if self.prices[ex]['last'] > 0) / max(1, len([ex for ex in self.exchanges if self.prices[ex]['last'] > 0]))
        ...
        try:
            # Use a lock to ensure thread safety for database access
            with threading.Lock():
                # Get the minimum number of data points across all exchanges
                ...
                for exchange in self.exchanges:
                    ...
                    # Store the data for this exchange
                    prices_data[exchange] = [row[1] for row in data]
                    
                    # Use timestamps from the first exchange
                ...
                if min_data_points > 0 and timestamps:
                    ...
                    try:
                        ...
                        try:
                            ...
                            # If timestamp format is unexpected, use index instead
                            timestamps = list(range(min_data_points))
                    
                    # Plot data for each exchange
                    for exchange in self.exchanges:
                        if len(prices_data[exchange]) >= min_data_points:
                            exchange_prices = prices_data[exchange][:min_data_points]
                            exchange_prices.reverse()  # Reverse to match chronological order
        ...
        return self.bid_bars, self.ask_bars, self.balance_text, self.arbitrage_text
