...
class CryptoArbitrageSimulator:
    ...
    # async def toggle(self, event=None):
    #     # global toggle_state
    #     self.toggle_state = not self.toggle_state  # Toggle the state
    #     if self.toggle_state:
    #         self.execute_button.label.set_text("Stop Exec")  # Change label to ON
    #         print("Execution has Started")
    #         self.execute_best_arbitrage()
    #         # while self.toggle_state:
    #         #     self.execute_best_arbitrage()
    #         #     await asyncio.sleep(1)
    #     else:
    #         self.execute_button.label.set_text("Start Exec")  # Change label to OFF
    ...
    def setup_plots(self):
        ...
        # Create empty lists for bid and ask bars
        self.bid_bars = []
        self.ask_bars = []
        
        # Create initial bars for each exchange
        for i, exchange in enumerate(self.exchanges_list):
            bid_bar = self.ax1.bar(i, 0, color='red', alpha=0.6, label='Bid' if i == 0 else "")
            ask_bar = self.ax1.bar(i, 0, color='green', alpha=0.6, label='Ask' if i == 0 else "", bottom=0)
            self.bid_bars.append(bid_bar[0])
            self.ask_bars.append(ask_bar[0])
        ...
        self.ax1.set_ylim(110000, 120000)
    ...
    def update_plots(self, frame=None):
        ...
        for i, exchange in enumerate(self.exchanges_list):
            if self.prices[exchange]['bid'] > 100 and self.prices[exchange]['ask'] > 100:
                bid_price = self.prices[exchange]['bid']
                ask_price = self.prices[exchange]['ask']
                
                # Update bid bar
                self.bid_bars[i].set_height(bid_price)
                self.bid_bars[i].set_y(0)
                
                # Update ask bar (spread visualization)
                spread = ask_price - bid_price
                self.ask_bars[i].set_height(spread)
                self.ask_bars[i].set_y(bid_price)
        ...
        try:
            with threading.Lock():
                ...
                for exchange in self.exchanges:
                    ...
                    if timestamps is None and data:
                ...
                if min_data_points > 0 and timestamps:
                    ...
                    self.ax4.set_ylabel('Price (USD)')
                    self.ax4.set_title('Price Disparity Across Exchanges')
                    self.ax4.set_ylim(110000, 120000)
                    self.ax4.legend()
